...
if(player.m_controller == KEYBOARD)
{
    ...
    if(keyboard_check_pressed(key_validate))
    {
        validate = true;
    }
    ...
    else if(shop.m_state == shop.BUY and keyboard_check_pressed(key_start))
    {
        shop.m_state = shop.START_RUN;
    }
}
else
{    
    var index = player.m_id;
    ...
    if(shop.m_state == shop.BUY and gamepad_button_check_pressed(player.m_controller, gp_start))
    {
        shop.m_state = shop.START_RUN;
    }
}

if(shop.m_state == self.BUY)
{
    /*
    SHOP: item array
    0 1 2      7  8  9
...
else if(shop.m_state == shop.ARE_U_SURE or shop.m_state == shop.START_RUN)
{
    shop.m_popup_answer += x_value;
    if(shop.m_popup_answer > 1)
    {
        shop.m_popup_answer = 0; 
    }
    else if(shop.m_popup_answer < 0)
    {
        shop.m_popup_answer = 1;    
    }
}
